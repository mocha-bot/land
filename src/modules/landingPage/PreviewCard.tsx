...
const SEED_MESSAGES: Record<string, Message[]> = {
  ...
  jq: [
    { id: 4, initial: 'S', name: 'suki', from: 'game jam', time: '1:05', body: 'is it okay to use premade assets for the jam?', avatarBg: 'linear-gradient(135deg, #f9c6d0, #b84a6a)' },
    { id: 5, initial: 'R', name: 'rio', from: 'indie devs', time: '1:07', body: 'yeah as long as you have a license. cc0 is safest.', avatarBg: 'linear-gradient(135deg, #b5ead7, #3a7a5a)' },
  ],
  'pixel-art/pa-general': [
    { id: 6, initial: 'K', name: 'kai', from: 'pixel art', time: '10:00', body: 'welcome to pixel art! share your work anytime 🎨', avatarBg: 'linear-gradient(135deg, #e0aaff, #7b2ff7)' },
  ],
  'pixel-art/pa-resources': [
    { id: 7, initial: 'A', name: 'amir', from: 'pixel art', time: '9:30', body: 'pinned: lospec.com for palettes, itch.io for assets', avatarBg: 'linear-gradient(135deg, #a0c4ff, #4a80c4)' },
  ],
  'game-jam/gj-general': [
    { id: 8, initial: 'L', name: 'leo', from: 'game jam', time: '8:00', body: 'jam starts in 3 days. theme votes close tonight!', avatarBg: 'linear-gradient(135deg, #b5ead7, #3a7a5a)' },
  ],
  'indie-devs/id-general': [
    { id: 9, initial: 'M', name: 'moss', from: 'indie devs', time: '11:15', body: 'devlog friday is back — drop your updates here', avatarBg: 'linear-gradient(135deg, #ffd6a5, #b87840)' },
  ],
  'indie-devs/id-showcase': [
    { id: 10, initial: 'K', name: 'kai', from: 'indie devs', time: '11:42', body: 'just dropped my first tileset pack 🎉 feedback welcome', avatarBg: 'linear-gradient(135deg, #e0aaff, #7b2ff7)' },
  ],
};

const BOT_REPLIES: Record<string, { initial: string; name: string; from: string; avatarBg: string; lines: string[] }> = {
  lnc: { initial: 'L', name: 'leo', from: 'game jam', avatarBg: 'linear-gradient(135deg, #b5ead7, #3a7a5a)', lines: ['good question! anyone else have thoughts?', 'i had the same issue last week 😅', 'this channel is so helpful ☕'] },
  jq: { initial: 'R', name: 'rio', from: 'indie devs', avatarBg: 'linear-gradient(135deg, #b5ead7, #3a7a5a)', lines: ['great point!', 'check the jam rules page too', 'yeah totally agree'] },
  default: { initial: 'M', name: 'moss', from: 'indie devs', avatarBg: 'linear-gradient(135deg, #ffd6a5, #b87840)', lines: ['hey!', '👋', 'nice'] },
};

const BRIDGED_META: Record<string, { serverCount: number; memberCount: number }> = {
...
let NEXT_ID = 200;
function getNextId() { NEXT_ID += 1; return NEXT_ID; }
...
// ─── Sub-components ────────────────────────────────────────────────────────────
function ServerIcon({ server, active, hasNotif, onClick }: { server: Server; active: boolean; hasNotif: boolean; onClick: () => void }) {
  return (
    <Tooltip label={server.name} placement='right' hasArrow openDelay={200}>
      <Box position='relative' display='flex' alignItems='center' cursor='pointer' onClick={onClick} pl='16px'>
        {/* Active pill */}
        <Box
          position='absolute'
          left='0'
      ...
      <Text fontSize='12px' color={TEXT_MUTED} fontStyle='italic'>
        ...
        {([0, 1, 2] as const).map((i) => (
          <Box
            ...
            sx={{ '@keyframes tpulse': { '0%, 100%': { opacity: 0.2 }, '50%': { opacity: 1 } } }}>
            ●
          </Box>
        ))}
      </Text>
...
function MsgRow({ msg }: { msg: Message }) {
  return (
    <Flex gap='10px' py='6px' px='16px' _hover={{ bg: 'rgba(255,255,255,0.02)' }} borderRadius='4px'>
...
export function PreviewCard() {
  ...
  const [channelBadges, setChannelBadges] = useState<Record<string, number>>({ 'gj-jq': 2, 'id-showcase': 1 });
  ...
  const handleSend = () => {
    ...
    // Notify other servers that share this bridged room
    if (activeChannel.bridgedRoomId) {
      const otherServerIds = SERVERS
        .filter((s) => s.id !== activeServerId && s.channels.some((c) => c.bridgedRoomId === activeChannel.bridgedRoomId))
        .map((s) => s.id);
      if (otherServerIds.length > 0) {
        setServerNotifs((prev) => {
          const next = { ...prev };
          otherServerIds.forEach((id) => { next[id] = true; });
          return next;
        });
      }
    }
    ...
    // Bot reply
    const botData = BOT_REPLIES[activeChannel.bridgedRoomId ?? ''] ?? BOT_REPLIES.default;
    setIsTyping(true);
    setTimeout(() => {
      setIsTyping(false);
      const reply: Message = {
  ...
  const bridgedMeta = activeChannel.bridgedRoomId ? BRIDGED_META[activeChannel.bridgedRoomId] : null;
  ...
  return (
    <Box
      ...
      <Grid templateColumns='72px 220px 1fr' h='calc(520px - 43px)'>
        ...
        {/* Channel list */}
        <Flex flexDir='column' bg={BG_CHANNELS} borderRight={`1px solid ${BORDER}`} overflowY='auto'>
          {/* Server name header */}
          <Flex
            align='center'
            ...
            {activeServer.channels.map((ch) => {
              ...
              return (
                <Flex
                  ...
                  _hover={{ bg: isActive ? 'rgba(255,220,0,0.1)' : 'rgba(255,255,255,0.05)', color: TEXT_PRIMARY }}
                  transition='all 0.1s'
                  ...
                  {badge > 0 && (
                    <Box bg='red.500' color='white' borderRadius='full' fontSize='10px' px='5px' py='1px' fontWeight={700} minW='18px' textAlign='center'>
                      {badge}
                    </Box>
                  )}
                </Flex>
        ...
        <Flex flexDir='column' bg={BG_MAIN} overflow='hidden'>
          ...
          <Flex
            ...
            {activeChannel.bridgedRoomId ? (
              ...
              <Text color='whiteAlpha.500' fontSize='16px' lineHeight='1'>#</Text>
            )}
            <Text fontSize='14px' fontWeight={600} color={TEXT_PRIMARY}>{activeChannel.name}</Text>
            {bridgedMeta && (
              <Flex align='center' gap='6px' ml='auto'>
                ...
                <Text fontSize='11px' color={TEXT_MUTED} fontFamily='mono'>
                  {bridgedMeta.serverCount} servers · {bridgedMeta.memberCount} members
                </Text>
              </Flex>
            )}
          </Flex>
          ...
          <Box
            ...
            {isTyping && <TypingIndicator name={BOT_REPLIES[activeChannel.bridgedRoomId ?? '']?.name ?? 'someone'} />}
          </Box>
          ...
          <Flex px='16px' py='12px' gap='8px' borderTop={`1px solid ${BORDER}`} flexShrink={0} align='center'>
            <Input
              ...
              _focus={{ borderColor: 'rgba(255,220,0,0.4)', boxShadow: '0 0 0 1px rgba(255,220,0,0.15)', outline: 'none' }}
